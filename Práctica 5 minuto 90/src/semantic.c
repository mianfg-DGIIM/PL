...
int isList(tData tipo_dato){
  return tipo_dato == LIST_INT || tipo_dato == LIST_FLOAT || tipo_dato == LIST_BOOLEAN || tipo_dato == LIST_CHAR;
}

void insertarEntrada(tIn te, char* nombre, tData tipo_dato, int nParam, DescriptorDeInstrControl* descp) {
  // Hacemos la entrada
  inTS entrada = {
    te,
    strdup(nombre),
...
void TS_AddMark() {
  ...
  if (esFunc == 1) {
    ...
    while (j > 0 && ts[j].entry == PARAM) {   // Mientras leamos parámetros formales los aniadimos
      ...
      insertarEntrada(VAR, ts[j].lex, ts[j].nParams, ts[j].nDim, ts[j].type );
...
void Check_PlusMinusBinary(attrs expr1, attrs op, attrs expr2, attrs *res) {
  ...
  if (!isList(expr1.type) && !isList(expr2.type)) {
    res->type = resType;
    res->nDim = 0;
  } else {
    ...
    if (isList(expr1.type) && !isList(expr2.type)) {
      ...
      res->nDim = expr1.nDim;
    } else if (!isList(expr1.type) && isList(expr2.type)) {
...
void Check_OpBinaryMul(attrs expr1, attrs op, attrs expr2, attrs *res) {
  if (op.attr == 0) {  // %
    ...
    if (expr1.type == INT && expr2.type == INT) {
      ...
      res->nDim = 0;
    // list % int
    } else if (isList(expr1.type) && expr2.type == INT) {
      res->type = expr1.type;
...
void Check_At(attrs expr1, attrs op, attrs expr2, attrs *res) {
  if (isList(expr1.type) && expr2.type == INT) {
...
void Check_ListTernary(attrs expr1, attrs op1, attrs expr2, attrs op2, attrs expr3, attrs *res) {
  if (!isList(expr1.type) || listToType(expr1.type) != expr2.type || expr3.type != INT) {
...
int esNumero(tData tipo_dato){
  return tipo_dato == INT || tipo_dato == FLOAT;
}